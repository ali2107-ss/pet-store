...
// Этот роут использует service role ключ на сервере для простоты.
// Требуется добавить SUPABASE_SERVICE_ROLE_KEY в окружение на сервере.

const url = process.env.NEXT_PUBLIC_SUPABASE_URL;
const serviceKey = process.env.SUPABASE_SERVICE_ROLE_KEY;

if (!url || !serviceKey) {
  console.warn('Cart API: missing SUPABASE_SERVICE_ROLE_KEY or NEXT_PUBLIC_SUPABASE_URL');
}
...
export async function GET(req: NextRequest) {
  try {
    ...
    const { data, error } = await supabaseAdmin.from('carts').select('items').eq('user_id', user_id).limit(1).single();
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    const { error } = await supabaseAdmin.from('carts').delete().eq('user_id', user_id);
    if (error) {
